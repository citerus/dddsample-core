...
import se.citerus.dddsample.infrastructure.persistence.jpa.entities.*;
...
public class SampleDataGenerator {
    ...
    public void generate() {
        ...
        //loadSampleData(jdbcTemplate, tt);
    ...
    private void loadHibernateData(TransactionTemplate tt, final HandlingEventFactory handlingEventFactory) {
        ...
        tt.execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ...
                for (LocationDTO location : SampleLocations.getAllDtos()) {
                    entityManager.persist(location);
                }

                for (VoyageDTO voyage : SampleVoyages.getAllDtos()) {
                    entityManager.persist(voyage);
                }

                // Cargo ABC123

                RouteSpecificationDTO routeSpecification = new RouteSpecificationDTO(HONGKONG_DTO, HELSINKI_DTO, toDate("2009-03-15"));
                TrackingId trackingId = new TrackingId("ABC123");
                List<LegDTO> itinerary = asList(
                        new LegDTO(HONGKONG_TO_NEW_YORK_DTO, HONGKONG_DTO, NEWYORK_DTO, toDate("2009-03-02"), toDate("2009-03-05")),
                        new LegDTO(NEW_YORK_TO_DALLAS_DTO, NEWYORK_DTO, DALLAS_DTO, toDate("2009-03-06"), toDate("2009-03-08")),
                        new LegDTO(DALLAS_TO_HELSINKI_DTO, DALLAS_DTO, HELSINKI_DTO, toDate("2009-03-09"), toDate("2009-03-12"))
                );
                HandlingActivityDTO nextExpectedActivity = new HandlingActivityDTO(HandlingEventDTO.Type.LOAD, HONGKONG_DTO, null);
                DeliveryDTO delivery = new DeliveryDTO(false, toDate("2009-03-12"), now, false,
                        RoutingStatus.ROUTED, nextExpectedActivity, TransportStatus.UNKNOWN, null, HONGKONG_DTO, null);
                CargoDTO abc123 = new CargoDTO(trackingId.idString(), HONGKONG_DTO, routeSpecification, itinerary, delivery);

//                abc123.assignToRoute(itinerary);
                entityManager.persist(abc123);

                HandlingEventDTO eventA1 = new HandlingEventDTO(
                        HONGKONG_DTO, abc123, toDate("2009-03-01"), HandlingEventDTO.Type.RECEIVE);
                entityManager.persist(eventA1);

                HandlingEventDTO eventA2 = new HandlingEventDTO(
                        HONGKONG_TO_NEW_YORK_DTO, HONGKONG_DTO, abc123, toDate("2009-03-02"), HandlingEventDTO.Type.LOAD);
                entityManager.persist(eventA2);

                HandlingEventDTO eventA3 = new HandlingEventDTO(
                        HONGKONG_TO_NEW_YORK_DTO, NEWYORK_DTO, abc123, toDate("2009-03-05"), HandlingEventDTO.Type.UNLOAD);
                entityManager.persist(eventA3);

                abc123.delivery.lastEvent = eventA3;
                abc123.delivery.currentVoyage = eventA3.voyage;
                abc123.delivery.eta = itinerary.get(itinerary.size()-1).unloadTime;
                abc123.delivery.transportStatus = TransportStatus.IN_PORT;

                entityManager.persist(abc123);

                // Cargo JKL567

                RouteSpecificationDTO routeSpecification1 = new RouteSpecificationDTO(HANGZOU_DTO, STOCKHOLM_DTO, toDate("2009-03-18"));
                TrackingId trackingId1 = new TrackingId("JKL567");
                List<LegDTO> itinerary1 = asList(
                        new LegDTO(HONGKONG_TO_NEW_YORK_DTO, HANGZOU_DTO, NEWYORK_DTO, toDate("2009-03-03"), toDate("2009-03-05")),
                        new LegDTO(NEW_YORK_TO_DALLAS_DTO, NEWYORK_DTO, DALLAS_DTO, toDate("2009-03-06"), toDate("2009-03-08")),
                        new LegDTO(DALLAS_TO_HELSINKI_DTO, DALLAS_DTO, STOCKHOLM_DTO, toDate("2009-03-09"), toDate("2009-03-11"))
                );
                HandlingActivityDTO nextExpectedActivity1 = new HandlingActivityDTO(HandlingEventDTO.Type.LOAD, HANGZOU_DTO, null);
                DeliveryDTO delivery1 = new DeliveryDTO(false, toDate("2009-03-11"), now, false,
                        RoutingStatus.MISROUTED, nextExpectedActivity1, TransportStatus.UNKNOWN, null, HANGZOU_DTO, null
                );
                CargoDTO jkl567 = new CargoDTO(trackingId1.idString(), HANGZOU_DTO, routeSpecification1, itinerary1, delivery1);
//                jkl567.assignToRoute(itinerary1);

                entityManager.persist(jkl567);

                HandlingEventDTO eventB1 = new HandlingEventDTO(
                        HANGZOU_DTO, jkl567, toDate("2009-03-01"), HandlingEventDTO.Type.RECEIVE
                );
                entityManager.persist(eventB1);

                HandlingEventDTO eventB2 = new HandlingEventDTO(
                        HONGKONG_TO_NEW_YORK_DTO, HANGZOU_DTO, jkl567, toDate("2009-03-03"), HandlingEventDTO.Type.LOAD
                );
                entityManager.persist(eventB2);

                HandlingEventDTO eventB3 = new HandlingEventDTO(
                        HONGKONG_TO_NEW_YORK_DTO, NEWYORK_DTO, jkl567, toDate("2009-03-05"), HandlingEventDTO.Type.UNLOAD
                );
                entityManager.persist(eventB3);

                HandlingEventDTO eventB4 = new HandlingEventDTO(
                        HONGKONG_TO_NEW_YORK_DTO, NEWYORK_DTO, jkl567, toDate("2009-03-06"), HandlingEventDTO.Type.LOAD
                );
                entityManager.persist(eventB4);
...
//                jkl567.deriveDeliveryProgress(handlingHistory1);
                jkl567.delivery.lastEvent = eventB4;
                jkl567.delivery.currentVoyage = eventB4.voyage;
                jkl567.delivery.eta = itinerary1.get(itinerary1.size()-1).unloadTime;
//                jkl567.delivery.transportStatus = TransportStatus.IN_PORT;
